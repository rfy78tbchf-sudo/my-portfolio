-- Explicit position events require a signed, source-backed delta. A balancing entry
-- without a KB source row is rejected. Legacy event names remain compatible.
alter table public.transactions add column if not exists position_delta numeric;
alter table public.transactions drop constraint if exists transactions_type_check;
alter table public.transactions add constraint transactions_type_check check (type = any(array[
  'buy','sell','dividend','interest','deposit','withdrawal','fee','tax','fx','other',
  ...
  'STOCK_DIVIDEND','RIGHTS_EVENT','OTHER_POSITION_ADJUSTMENT']));
alter table public.transactions add constraint source_backed_position_delta_check check (
  type not in('TRANSFER_IN','TRANSFER_OUT','ACCOUNT_TRANSFER','BROKER_TRANSFER_IN','BROKER_TRANSFER_OUT',
    'STOCK_SPLIT','REVERSE_SPLIT','MERGER','SPINOFF','SYMBOL_CHANGE','STOCK_DIVIDEND','RIGHTS_EVENT','OTHER_POSITION_ADJUSTMENT')
  or (position_delta is not null and position_delta<>0 and quantity>0 and source='api'
      and provider_payload is not null and jsonb_typeof(provider_payload)='object'
...
begin
  ...
  with raw_codes as materialized (
    ...
    select i.asset_key,t.trade_at,t.external_id,t.type,t.quantity,
      ...
      case when t.position_delta is not null then t.position_delta
    ...
    where t.account_id=v_account and t.quantity>0 and t.type in
      ('buy','sell','transfer_in','transfer_out','split_in','split_out','corporate_in','corporate_out','TRANSFER_IN','TRANSFER_OUT','ACCOUNT_TRANSFER','BROKER_TRANSFER_IN','BROKER_TRANSFER_OUT','STOCK_SPLIT','REVERSE_SPLIT','MERGER','SPINOFF','SYMBOL_CHANGE','STOCK_DIVIDEND','RIGHTS_EVENT','OTHER_POSITION_ADJUSTMENT')
  ), ledger as (
