-- Guard mixed days: validate each broker row without using ledger row order as execution order.
      ...
      if v_mixed then
        ...
        loop
          ...
          if v_trade.type='sell' and v_day.d between v_start and v_today then
            v_count:=v_count+1;
            if v_valid then v_order:=v_order+1;else v_source_missing:=v_source_missing+1;end if;
