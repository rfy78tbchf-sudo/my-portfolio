-- KB can show a dated cash deposit in the ledger before its valuation enters
-- the saved account balance. Allocate it to the first corroborating snapshot.
-- This is derived on every read; the broker's original transaction is intact.
create or replace function public.cash_flow_balance_date(p_flow_id uuid)
returns date language plpgsql stable security invoker set search_path = '' as $$
declare
  v_flow public.cash_flows%rowtype;
  v_day date;
  v_prev public.daily_account_snapshots%rowtype;
  v_book public.daily_account_snapshots%rowtype;
  v_next public.daily_account_snapshots%rowtype;
  v_amount numeric;
  v_tolerance numeric;
begin
  select * into v_flow from public.cash_flows where id=p_flow_id;
  if not found or v_flow.source<>'api' or v_flow.currency<>'KRW'
     or abs(v_flow.amount)<100000 then return null; end if;
  v_day := (v_flow.occurred_at at time zone 'Asia/Seoul')::date;
  v_amount := v_flow.amount*coalesce(v_flow.fx_rate_to_base,1);
  select * into v_book from public.daily_account_snapshots
    where account_id=v_flow.account_id and snapshot_date=v_day;
  if not found or v_flow.occurred_at>v_book.snapshot_at then return null; end if;
  select * into v_prev from public.daily_account_snapshots
    where account_id=v_flow.account_id and snapshot_date<v_day
    order by snapshot_date desc limit 1;
  if not found then return null; end if;
  select * into v_next from public.daily_account_snapshots
    where account_id=v_flow.account_id and snapshot_date>v_day
      and snapshot_date<=v_day+3
    order by snapshot_date limit 1;
  if not found then return null; end if;
  -- Do not infer the timing when other flows could explain the same balance.
  if exists (select 1 from public.cash_flows c
      where c.account_id=v_flow.account_id and c.id<>v_flow.id
        and (c.occurred_at at time zone 'Asia/Seoul')::date
            between v_day and v_next.snapshot_date)
     or exists (select 1 from public.manual_adjustments m
      where m.account_id=v_flow.account_id and m.active and m.kind='external_flow'
        and m.occurred_on between v_day and v_next.snapshot_date) then
    return null;
  end if;
  v_tolerance:=greatest(120000,abs(v_amount)*0.04);
  -- Strip the broker's marked-to-market P&L from each asset change. The
  -- cash-like remainder is absent on the ledger day and appears next time.
  if abs((v_book.total_assets-v_prev.total_assets)
      -(coalesce(v_book.unrealized_pnl,0)-coalesce(v_prev.unrealized_pnl,0)))<=v_tolerance
     and abs((v_next.total_assets-v_book.total_assets)
      -(coalesce(v_next.unrealized_pnl,0)-coalesce(v_book.unrealized_pnl,0))
      -v_amount)<=v_tolerance then
    return v_next.snapshot_date;
  end if;
  return null;
end $$;
revoke all on function public.cash_flow_balance_date(uuid) from public,anon;
grant execute on function public.cash_flow_balance_date(uuid) to authenticated;

create or replace function public.allocated_external_flow(
  p_account_id uuid,p_previous_at timestamptz,p_previous_date date,
  p_snapshot_at timestamptz,p_snapshot_date date)
returns numeric language sql stable security invoker set search_path = '' as $$
  select coalesce(sum(c.amount*coalesce(c.fx_rate_to_base,1)),0)
  from public.cash_flows c
  where c.account_id=p_account_id and (
    case when public.cash_flow_balance_date(c.id) is not null then
      public.cash_flow_balance_date(c.id)>coalesce(p_previous_date,p_snapshot_date-1)
      and public.cash_flow_balance_date(c.id)<=p_snapshot_date
    else c.occurred_at>coalesce(p_previous_at,p_snapshot_date::timestamp at time zone 'Asia/Seoul')
      and c.occurred_at<=p_snapshot_at end
  );
$$;
revoke all on function public.allocated_external_flow(uuid,timestamptz,date,timestamptz,date) from public,anon;
grant execute on function public.allocated_external_flow(uuid,timestamptz,date,timestamptz,date) to authenticated;


-- regenerated functions follow
...
begin
  ...
  select
    public.allocated_external_flow(new.account_id,v_previous_at,v_previous_date,new.snapshot_at,new.snapshot_date)
...
begin
  ...
  loop
    select
      public.allocated_external_flow(p_account_id,v_previous_at,v_previous_date,v_snapshot.snapshot_at,v_snapshot.snapshot_date)
      + coalesce((select sum(ma.amount)
        from public.manual_adjustments ma
...
begin
  ...
  if v_base.id is not null and v_end.id is not null and v_end.snapshot_date>v_base.snapshot_date then
    ...
    select
      public.allocated_external_flow(v_account,v_base.snapshot_at,v_base.snapshot_date,v_end.snapshot_at,v_end.snapshot_date)
      +
      coalesce((select sum(ma.amount)
        from public.manual_adjustments ma
    ...
    daily as (
      select o.snapshot_date,o.snapshot_at,o.prev_date,o.prev_at,o.prev_assets,o.total_assets,
        public.allocated_external_flow(v_account,o.prev_at,o.prev_date,o.snapshot_at,o.snapshot_date)
        +
        coalesce((select sum(ma.amount)
          from public.manual_adjustments ma
          where ma.user_id=v_user and ma.account_id=v_account
  ...
  return jsonb_build_object(
    ...
    'delayed_deposit_count', (select count(*) from public.cash_flows cf
      where cf.account_id=v_account and cf.type='deposit'
        and public.cash_flow_balance_date(cf.id) is not null
        and public.cash_flow_balance_date(cf.id)>v_base.snapshot_date
        and public.cash_flow_balance_date(cf.id)<=v_end.snapshot_date),
...
daily as (
  select o.snapshot_date,o.snapshot_at,o.prev_date,o.prev_at,o.prev_assets,o.total_assets,
    public.allocated_external_flow(o.account_id,o.prev_at,o.prev_date,o.snapshot_at,o.snapshot_date)
    +
    coalesce((select sum(ma.amount)
      from public.manual_adjustments ma
...
begin
  ...
  if v_summary->>'return_ready' is distinct from 'true' and v_end is not null then
    ...
    if v_base is not null then
      ...
      order by s.snapshot_at desc limit 1;
      select public.allocated_external_flow(v_account,v_base_at,v_base,v_end_at,v_end)
...
-- Rebuild derived performance only; original broker records are unchanged.
do $$
declare v_account record;
begin
  for v_account in select account_id,min(snapshot_date) first_date
    from public.daily_account_snapshots group by account_id loop
