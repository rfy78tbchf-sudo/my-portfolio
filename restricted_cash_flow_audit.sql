-- Keep privileged balance repair outside the API-exposed public schema.
create schema if not exists portfolio_internal;
revoke all on schema portfolio_internal from public,anon;
grant usage on schema portfolio_internal to authenticated;
...
revoke all on function portfolio_internal.reconcile_live_cash_flows_backend() from public,anon;
grant execute on function portfolio_internal.reconcile_live_cash_flows_backend() to authenticated;

create or replace function public.reconcile_live_cash_flows()
returns jsonb language sql security invoker set search_path = '' as $$
  select portfolio_internal.reconcile_live_cash_flows_backend();
$$;
