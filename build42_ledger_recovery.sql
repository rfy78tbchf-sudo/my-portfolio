-- Build 42. Historical KB IDs remain untouched; instrument identity is joined
-- by the provider's ISIN. The private build42_prechange_backup is made first.
alter table public.transactions drop constraint if exists transactions_type_check;
alter table public.transactions add constraint transactions_type_check check
  (type in ('buy','sell','dividend','interest','deposit','withdrawal','fee','tax','fx','other',
    'transfer_in','transfer_out','split_in','split_out','corporate_in','corporate_out'));

-- Only explicit provider share-movement descriptions are converted. Cash
-- adjustments and unsupported rights events stay 'other' for manual review.
update public.transactions t set type=case
  when t.provider_payload->>'smry_nm' ~ '(액면분할|액면병합)' and t.provider_payload->>'smry_nm' ~ '입고' then 'split_in'
  when t.provider_payload->>'smry_nm' ~ '(액면분할|액면병합)' and t.provider_payload->>'smry_nm' ~ '출고' then 'split_out'
  when t.provider_payload->>'smry_nm' ~ '(상환|종목변경|합병|스핀오프)' and t.provider_payload->>'smry_nm' ~ '입고' then 'corporate_in'
  when t.provider_payload->>'smry_nm' ~ '(상환|종목변경|합병|스핀오프)' and t.provider_payload->>'smry_nm' ~ '출고' then 'corporate_out'
  when t.provider_payload->>'smry_nm' ~ '입고' then 'transfer_in'
  when t.provider_payload->>'smry_nm' ~ '출고' then 'transfer_out'
  else t.type end,
  updated_at=now()
where t.source='api' and t.type='other' and t.quantity>0
  and t.security_id is not null and t.provider_payload->>'smry_nm' ~ '(입고|출고)';

-- Attach authoritative provider identifiers without merging IDs or damaging
-- existing investment thesis references. Ambiguous identities are skipped.
with unique_isin as (
  select t.security_id,min(trim(t.provider_payload->>'stnd_is_cd')) isin
  from public.transactions t
  where t.security_id is not null
    and trim(t.provider_payload->>'stnd_is_cd') ~ '^[A-Z]{2}[A-Z0-9]{10}$'
  group by t.security_id having count(distinct trim(t.provider_payload->>'stnd_is_cd'))=1
)
update public.securities s set isin=u.isin,updated_at=now()
from unique_isin u where s.id=u.security_id and s.isin is distinct from u.isin;

create index if not exists transactions_account_provider_isin_date_idx
  on public.transactions(account_id,(provider_payload->>'stnd_is_cd'),trade_at)
  where type in ('buy','sell','transfer_in','transfer_out','split_in','split_out','corporate_in','corporate_out');
create index if not exists securities_isin_lookup_idx on public.securities(isin) where isin is not null;
